...
$(document).ready(function () {
    //if hub is not declared
    if (consolidationHub == undefined)
    {
    ...
    $('#stagingToteID').keypress(function (e) {
        if (e.which == 13) {
           ...
           var input = $('.col-xs-5 input[value="' + $(this).val() + '"]').parent().parent().find('.location-input').focus().val('');
        }
    ...
    function appendStagingRow(row) {
        appendstring = '<div class="row" style="margin-top:10px;">' +
                   '<div class="col-xs-5" name="toteID"><input disabled=disabled maxlength="50" name="" value="' + row[0] + '" class="form-control" /></div>' +
                   '<div class="col-xs-5" name="location"><input data-trigger="focus" data-content="Current Loc: ' + row[2] + '" data-placement="top" maxlength="50" name="" value="' + row[2] + '" class="form-control location-input" data-val="' + row[2] + '" onfocus="this.value = this.value" /></div>' +
                   '<div class="col-xs-1" name="remove"><button type="button" class="btn btn-danger clear-location" data-toggle="tooltip" data-placement="top" data-original-title="Delete"><span class="glyphicon glyphicon-remove"></span></button></div>';
